...
# İlanların tutulduğu geçici hafıza (veya veritabanı bağlantınız)
ilanlar = []
...
<head>
    ...
    <style>
        ...
        .price { font-size: 16px; font-weight: bold; color: #d9534f; width: 150px; }

        /* Modal (Açılır Pencere) Stili */
        .modal-bg { display: none; position: fixed; top: 0; left: 0; width: 100%; height: 100%; background: rgba(0,0,0,0.6); z-index: 1000; align-items: center; justify-content: center; }
        ...
        .modal-text { white-space: pre-wrap; line-height: 1.6; font-size: 14px; background: #f8f9fa; padding: 15px; border-radius: 6px; border: 1px solid #e9ecef; }
    </style>
</head>
<body>
...
<script>
        ...
        try {
            ...
            tumIlanlar.forEach((item, index) => {
                ...
                card.innerHTML = `
                    ${imgHTML}
                    <div class="details">
                        <div class="title">${item.detay.substring(0, 90)}...</div>
            ...
            console.error("İlanlar alınamadı:", e);
        }
    }
    ...
    function detayAc(index) {
        ...
        document.getElementById('modalMetin').innerText = item.detay;
